...
public class MenuDao {
	...
	public ArrayList<MenuVo> selectAll() {
		...
		try {
			...
			while (rs.next()) {
				list.add(new MenuVo(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5),
						rs.getString(6)));
			}
	...
	// 카테고리별 제품 select
	public ArrayList<MenuVo> selectByCategory(String menu) {
		ArrayList<MenuVo> list = new ArrayList<>();
		Connection conn = dbconn.conn();
		String sql = "select * from Menu where category like '%' || ? || '%' order by num desc";
		MenuVo vo = null;
	...
	public MenuVo selectByNum(int n) {
		...
		try {
			...
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
				vo = new MenuVo(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5),
						rs.getString(6));
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
